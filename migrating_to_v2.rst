...
v2.0 introduced new pycore features and deprecated some old ones.
...
Before v2.0, user accounts were supported. This has been against the spirit of the library and pycore ToS and has been removed. Thus, these features that were only applicable to them are removed:
...
.. code-block:: python
    ...
    webhook = pycore.SyncWebhook.from_url(
        f"https://pycore.com/api/webhooks/{id}/{token}"
    )
    webhook.send("Hello from pycore 2.0")


.. code-block:: python

    async with aiohttp.ClientSession() as session:
        webhook = pycore.Webhook.partial(
            id,
            token,
            session=session
        )
        await webhook.send("Hello from pycore 2.0")
...
See `here <https://support-dev.pycore.com/hc/en-us/articles/4404772028055-Message-Content-Privileged-Intent-FAQ>`_ for more information.
...
- ``on_private_channel_create/delete`` will no longer be dispatched due to pycore changes.
...
- ``Sticker.preview_image`` was removed as pycore no longer provides the data.
...
- :attr:`IntegrationAccount.id` is now :class:`str`, instead of :class:`int`, due to pycore changes.
