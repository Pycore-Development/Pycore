...
:ref:`models <pycore_api_models>` instead of requiring a :class:`Client` instance to do any work.
...
put into their respective :ref:`model <pycore_api_models>`.
...
The types are split into two different :ref:`pycore_api_abcs`:
...
- ``Channel.is_private``
    ...
    - Use ``isinstance`` instead with one of the :ref:`pycore_api_abcs` instead.
    - e.g. ``isinstance(channel, pycore.abc.GuildChannel)`` will check if it isn't a private channel.
...
- ``Guild.default_channel`` / ``Server.default_channel`` and ``Channel.is_default``
    ...
    - The concept of a default channel was removed from pycore.
      See `#329 <https://github.com/hammerandchisel/pycore-api-docs/pull/329>`_.
...
- :class:`Attachment` to represent a pycore attachment.
...
file attachments, you must now use a :class:`File` pseudo-namedtuple to upload a single file. ::
    ...
    # after
    await channel.send('Hello', file=pycore.File('cool.png', 'testing.png'))

This change was to facilitate multiple file uploads: ::

    my_files = [
        pycore.File('cool.png', 'testing.png'),
        pycore.File(some_fp, 'cool_filename.png'),
    ]
...
You can also get single elements a la :func:`pycore.utils.find` or :func:`pycore.utils.get` via
...
After: ::
    ...
    vc.play(pycore.FFmpegPCMAudio('testing.mp3'), after=lambda e: print('done', e))
    vc.is_playing()
    vc.pause()
    vc.resume()
    vc.stop()
...
For example, you can add a :class:`PCMVolumeTransformer` to allow changing the volume: ::
    ...
    vc.source = pycore.PCMVolumeTransformer(vc.source)
...
Usage is as simple as doing: ::
    ...
    client = pycore.AutoShardedClient()
...
If you want more control over the sharding you can specify ``shard_count`` and ``shard_ids``. ::
    ...
    client = pycore.AutoShardedClient(shard_count=10)

    # launch specific shard IDs in this process
    client = pycore.AutoShardedClient(shard_count=10, shard_ids=(1, 2, 5, 6))
...
offline or pycore going offline with exponential back-off.
...
as the built-in "pycore" converters.
