...
This is a list of Frequently Asked Questions regarding using ``pycore`` and its extension modules. Feel free to suggest a
...
Example code can be found in the `examples folder <https://github.com/Pycore-Development/pycore/tree/master/examples>`_
in the repository.
...
Putting both of these pieces of info together, you get the following: ::
    ...
    client = pycore.Client(activity=pycore.Game(name='my game'))

    # or, for watching:
    activity = pycore.Activity(name='my activity', type=pycore.ActivityType.watching)
    client = pycore.Client(activity=activity)
...
.. code-block:: python3
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(my_url) as resp:
            ...
            await channel.send(file=pycore.File(data, 'cool_image.png'))
...
Quick example: ::
    ...
    # no ID, do a lookup
    emoji = pycore.utils.get(guild.emojis, name='LUL')
    if emoji:
        await message.add_reaction(emoji)
...
this together we can do the following: ::
    ...
    voice.play(pycore.FFmpegPCMAudio(url), after=my_after)

How do I run something in the background?
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

`Check the background_task.py example. <https://github.com/Pycore-Development/pycore/blob/master/examples/background_task.py>`_
...
Quick example: ::
    ...
    guild = pycore.utils.get(client.guilds, name='My Server')

    # make sure to check if it's found
    if guild is not None:
        # find a channel by name
        channel = pycore.utils.get(guild.text_channels, name='cool-channel')
...
pycore special-cases uploading an image attachment and using it within an embed so that it will not
...
Quick example: ::
    ...
    file = pycore.File("path/to/my/image.png", filename="image.png")
    embed = pycore.Embed()
...
Questions regarding ``pycore.ext.commands`` belong here.
