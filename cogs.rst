...
- Every command is marked with the :func:`pycore.command` decorator or the corresponding shortcut command decorator.
...
This example cog defines a ``Greetings`` category for your commands, with a single slash command named ``hello`` as well as a listener to listen to an :ref:`Event <pycore-api-events>`.

.. code-block:: python3

    class Greetings(pycore.Cog):
        ...
        @pycore.slash_command()
        async def hello(self, ctx, *, member: pycore.Member = None):
            """Says hello"""
