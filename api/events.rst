...
.. code-block:: python
    ...
    @client.event
    async def on_message(message: pycore.Message):
...
.. function:: on_application_command_error(context, exception)
    ...
    :param exception: The pycoreException associated to the error.
    :type exception: :class:`pycoreException`
...
.. function:: on_connect()
    ...
    Called when the client has successfully connected to pycore. This is not
...
.. function:: on_shard_connect(shard_id)
    ...
    to denote when a particular shard ID has connected to pycore.
...
.. function:: on_disconnect()
    ...
    Called when the client has disconnected from pycore, or a connection attempt to pycore has failed.
    This could happen either through the internet being disconnected, explicit calls to close,
    or pycore terminating the connection one way or the other.
...
.. function:: on_shard_disconnect(shard_id)
    ...
    to denote when a particular shard ID has disconnected from pycore.
...
.. function:: on_ready()
    ...
    Called when the client is done preparing the data received from pycore. Usually after login is successful
...
.. function:: on_socket_event_type(event_type)
    ...
    from the pycore gateway.

    .. versionadded:: 2.0

    :param event_type: The event type from pycore that is received, e.g. ``'READY'``.
...
.. function:: on_interaction(interaction)
    ...
    .. warning::
        ...
        with the :class:`~pycore.ui.View` instead as it provides a nicer user experience.
...
.. function:: on_invite_delete(invite)
    ...
    .. note::
        ...
        filled by the pycore gateway for this event is :attr:`Invite.code`.
...
.. function:: on_raw_message_edit(payload)
    ...
    the raw data given by the `gateway <https://pycore.com/developers/docs/topics/gateway#message-update>`_.

    Since the data payload can be partial, care must be taken when accessing stuff in the dictionary.
    One example of a common case of partial data is when the ``'content'`` key is inaccessible. This
    denotes an "embed" only edit, which is an edit in which only the embeds are updated by the pycore
    embed server.
...
.. function:: on_reaction_add(reaction, user)
    ...
    .. note::
        ...
        but due to pycore not providing updated user information in a direct message
