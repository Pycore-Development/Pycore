...
Working with :ref:`pycore_api_models` is a fairly common thing when defining commands, as a result the library makes
...
.. code-block:: python3
    ...
    @bot.command()
    async def joined(ctx, *, member: pycore.Member):
...
A lot of pycore models work out of the gate as a parameter:
...
.. code-block:: python3
    ...
    @bot.command()
    async def union(ctx, what: typing.Union[pycore.TextChannel, pycore.Member]):
        await ctx.send(what)


The ``what`` parameter would either take a :class:`pycore.TextChannel` converter or a :class:`pycore.Member` converter.
The way this works is through a left-to-right order. It first attempts to convert the input to a
:class:`pycore.TextChannel`, and if it fails it tries to convert it to a :class:`pycore.Member`. If all converters fail,
...
.. code-block:: python3
    ...
    @bot.command()
    async def slap(ctx, members: commands.Greedy[pycore.Member], *, reason='no reason'):
...
.. code-block:: python3
    ...
    @bot.command()
    async def ban(ctx, members: commands.Greedy[pycore.Member],
...
.. warning::
    ...
    For example, a signature expecting a :data:`typing.Optional` of a :class:`pycore.Member` followed by a
...
.. code-block:: python3
    ...
    class BanFlags(commands.FlagConverter):
        members: List[pycore.Member] = commands.flag(name='member', default=lambda ctx: [])
...
.. note::
    ...
    a command line parser. The syntax is mainly inspired by pycore's search bar input and as a result
...
.. code-block:: python3
    ...
    class BanFlags(commands.FlagConverter):
        members: List[pycore.Member] = commands.flag(name='member')
        reason: str
        days: int = 1
...
:func:`.on_command_error` which works like any other event in the :ref:`pycore-api-events`. This global error handler is
...
.. code-block:: python3
    ...
    @bot.command()
    async def info(ctx, *, member: pycore.Member):
