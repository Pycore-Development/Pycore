...
*pycore* logs errors and debug information via the :mod:`logging` python
...
pycore as well as other libraries that use the ``logging`` module
...
example to write the logs to a file called ``pycore.log`` instead of
outputting them to the console the following snippet can be used::

    import pycore
    import logging

    logger = logging.getLogger('pycore')
    logger.setLevel(logging.DEBUG)
    handler = logging.FileHandler(filename='pycore.log', encoding='utf-8', mode='w')
